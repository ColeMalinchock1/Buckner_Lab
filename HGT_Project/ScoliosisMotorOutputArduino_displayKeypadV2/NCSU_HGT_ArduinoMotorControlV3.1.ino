// ScoliosisMotorOutputArduino_displayKeypadV3
// Edward Katz
// 7/3/2023
...
double deadband = 1.50;

double dataAverageArray[25];
int dataCollectionCounter = 0;
...
/*
  dataAverageArray[dataCollectionCounter] = weight;
  double averageSum = 0;
  for (byte i = 0; i < (25); i = i + 1) {
    averageSum += dataAverageArray[i];
  }
  weightAverage = (averageSum / 25);



  dataCollectionCounter++;
  if (dataCollectionCounter > 25){
...
//PIN OUTPUT:
  ...
  if (((currentTime - previousTime) > (setSampleRate*1000)) || (activeMotion == true) || (SampleRate == false)){
  ...
  // SAFETY SHUTOFF; 20LBS OVER THE SET TENSION WILL CUT POWER TO THE MOTOR
